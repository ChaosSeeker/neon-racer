...
function updateLandscapeLock(){
  if (!isTouch) {
    ...
    ui.rotateOverlay?.setAttribute("aria-hidden", "true");
  ...
  if (!ok) {
    ...
    ui.rotateOverlay?.setAttribute("aria-hidden", "false");
...
document.getElementById("btn-rot-ok")?.addEventListener("click", () => {
  // Dismiss the prompt (it will reappear if still portrait)
  document.body.classList.remove("landscape-lock");
  ui.rotateOverlay?.setAttribute("aria-hidden", "true");
});
...
function loop(now) {
  ...
  // enforce landscape for touch devices during gameplay
  const okLandscape = updateLandscapeLock();
  if (!okLandscape && (mode === "run" || mode === "countdown")) {
    // Freeze gameplay until rotated.
    renderer.render(dt);
