...
export class Renderer3D {
  ...
  setMode(mode) {
    ...
    if (mode === "home") {
      ...
      this.camGame.position.set(0, 2.3, 6.2);
  ...
  sync(state) {
    ...
    this.camGame.position.z = lerp(this.camGame.position.z, 6.2 + targetZ, 0.06);
  ...
  makeRoad() {
    ...
    const road = new THREE.Mesh(new THREE.PlaneGeometry(12, 160), roadMat);
    ...
    const stripGeo = new THREE.BoxGeometry(0.10, 0.02, 160);
    const left = new THREE.Mesh(stripGeo, edgeMat);
    left.position.set(-4.1, 0.02, -70);
    g.add(left);

    const right = new THREE.Mesh(stripGeo, edgeMat);
    right.position.set(4.1, 0.02, -70);
    g.add(right);
  ...
  makeLaneLines() {
    ...
    const segGeo = new THREE.BoxGeometry(0.08, 0.01, 1.8);
    for (const x of [-1.35, 1.35]) {
      for (let i = 0; i < 42; i++) {
        const m = new THREE.Mesh(segGeo, mat);
        m.position.set(x, 0.021, -6 - i * 3.6);
        g.add(m);
      }
    }
    return g;
  ...
  makeRails() {
    ...
    const railGeo = new THREE.BoxGeometry(0.18, 0.20, 160);
    const l = new THREE.Mesh(railGeo, mat);
    l.position.set(-4.6, 0.1, -70);
    l.castShadow = true;
    g.add(l);

    const r = new THREE.Mesh(railGeo, mat);
    r.position.set(4.6, 0.1, -70);
    r.castShadow = true;
    g.add(r);
