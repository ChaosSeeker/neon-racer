...
(() => {
  ...
  async function startGame() {
    ...
    // IMPORTANT: show the game screen BEFORE measuring canvas size.
    // If the canvas (or its parent) is display:none, getBoundingClientRect() becomes 0 and the game draws "blank".
    showScreen("game");
    screens.gameOver.classList.add("hidden");
    screens.countdown.classList.remove("hidden");

    // size canvas on the next frame when layout is ready
    await new Promise(r => requestAnimationFrame(() => { resizeGameCanvas(); r(); }));
  ...
  function modalBodyFor(id) {
    const card = (img, title, sub, rightHtml="") => `
      <div class="itemCard">
        <div class="itemLeft">
          <img class="itemImg" src="${img}" alt="${escapeHtml(title)}"/>
          <div class="itemText">
            <div class="itemTitle">${escapeHtml(title)}</div>
            <div class="itemSub">${sub}</div>
          </div>
        </div>
        <div class="itemRight">${rightHtml}</div>
      </div>
    `;
    ...
    switch (id) {
      ...
      case "garageModal": {
        return `
          <p class="modalHint">Select your car (more coming soon).</p>
          ${card("assets/player_car.png","Rookie Racer (Default)","Owned","<span class='pill owned'>OWNED</span> <button id='selectDefaultCarBtn' class='primary small'>Select</button>")}
          <div class="divider"></div>
          <p class="modalHint">Traffic / Obstacles in this season:</p>
          ${card("assets/obstacle_yellow.png","Yellow Car","Traffic","<span class='pill'>Obstacle</span>")}
          ${card("assets/obstacle_bus.png","Red Bus","Traffic","<span class='pill'>Obstacle</span>")}
          ${card("assets/obstacle_bike.png","Motorcycle","Traffic","<span class='pill'>Obstacle</span>")}
        `;
      }

      case "shopModal":
        return `
          <p class="modalHint">Spend coins to unlock items. (UI ready — content can expand.)</p>
          ${card("assets/avatar_placeholder.png","Starter Bundle","Cosmetics + 250 coins","<button class='primary small' disabled>Buy 199</button>")}
          ${card("assets/avatar_placeholder.png","Neon UnderGlow","Visual upgrade","<button class='primary small' disabled>Buy 120</button>")}
          ${card("assets/avatar_placeholder.png","Engine FX Pack","Audio/FX upgrade","<button class='primary small' disabled>Buy 90</button>")}
          <p style="margin-top:10px; color:rgba(255,255,255,.75); font-weight:700;">Tip: We can plug real purchase/unlock logic next (localStorage or Supabase).</p>
        `;

      case "missionsModal":
        return `
          <p class="modalHint">Missions (sample list):</p>
          <div class="missionGrid">
            <div class="missionCard"><b>Rookie Warmup</b><div>Play 1 run</div><div class="missionReward">Reward: +25 coins</div></div>
            <div class="missionCard"><b>Traffic Dodger</b><div>Dodge 10 vehicles</div><div class="missionReward">Reward: +40 coins</div></div>
            <div class="missionCard"><b>Score Hunter</b><div>Reach 500 score</div><div class="missionReward">Reward: +60 coins</div></div>
          </div>
        `;
